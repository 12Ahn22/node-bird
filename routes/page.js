...
router.use((req, res, next) => {
  ...
  res.locals.user = req.user; // 로그인 시, users에는 로그인한 유저 정보가 들어있다.-> passport 미들웨어가 넘겨준다.
  res.locals.followerCount = 0;
  res.locals.followingCount = 0;
  res.locals.followerIdList = [];

  console.log('===============req.locals============', res.locals); // 지금 데이터가 안들어가고 있음.
  next();
});
